...
public class HelloAR
{
    ...
    public void setTarget(int width, int height, int screenrotation, float x, float y){
        Vec2F screenxy = new Vec2F(x/width,y/height);
        //Vec2F screenxy = new Vec2F((float) Math.pow((x/width),2),(float) Math.pow((y/height),2));

        OutputFrame oframe = outputFrameBuffer.peek();
        if (oframe == null) { Log.d("artx", "oframe null"); return;}
        InputFrame iframe = oframe.inputFrame();
        if (iframe == null) { Log.d("artx", "iframe null"); oframe.dispose(); return; }
        CameraParameters cameraParameters = iframe.cameraParameters();
        //CameraParameters cameraParameters = camera.cameraParameters();
        if (cameraParameters == null) { Log.d("artx", "cameraParameters null"); oframe.dispose(); iframe.dispose(); return; }
        float aspect_ratio = (float)width / (float)height;
        Vec2F vec = cameraParameters.
                imageCoordinatesFromScreenCoordinates(aspect_ratio,screenrotation,true,false,screenxy);
        Log.d("artx","screenrotation: " + screenrotation + " aspect ratio: " + aspect_ratio + " vec: " +
                vec.data[0] + " | " + vec.data[1]);

        tracker.alignTargetToCameraImagePoint(vec);
    }
        ...
        try {
            ...
            for (FrameFilterResult oResult : oframe.results()) {
                if (oResult instanceof SurfaceTrackerResult) {
                    ...
                    boxRenderer.render(projectionMatrix, gluInvertMatrix(transform), new Vec3F(0.1f, 0.1f, 0.1f));
                }
                if (oResult != null) {
                    oResult.dispose();
                }
